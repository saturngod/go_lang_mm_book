...
*   `log.Println()`: Standard logger ကို အသုံးပြု၍ standard error (stderr) သို့ timestamp နှင့်တကွ စာသားကို print ထုတ်သည်။
*   `log.Printf()`: Standard logger ကို အသုံးပြု၍ format specifier များနှင့်အတူ standard error (stderr) သို့ print ထုတ်သည်။
*   `log.Fatal()`: Standard logger ကို အသုံးပြု၍ error message ကို standard error (stderr) သို့ print ထုတ်ပြီးနောက် program ကို ချက်ချင်းရပ်တန့်သည် (`os.Exit(1)` ကို ခေါ်သည်)။
