...
`database/sql/driver` package သည် database driver များ implement လုပ်ရမည့် `driver.Driver`, `driver.Conn` ကဲ့သို့သော interface များကို **သတ်မှတ်ပေးထားသည်**။ `database/sql` package သည် ထို interfaces များကို အသုံးပြု၍ driver များနှင့် ဆက်သွယ်ပြီး MySQL သို့မဟုတ် PostgreSQL နှင့် မည်သို့ချိတ်ဆက်ရမည်ကို ကိုယ်တိုင် **implement မလုပ်ပါ**။

ထိုအစား၊ third-party driver developer များ (ဥပမာ- `go-sql-driver/mysql`) က ထို `driver.Driver` interface ကို သူတို့၏ database အတွက် implement လုပ်ထားသော concrete type များကို ရေးသားပေးပါသည်။
...
graph TD
    ...
    subgraph "Go Standard Library"
        StdSQL["database/sql/driver package<br/>(Defines Driver interface)"]
    end
